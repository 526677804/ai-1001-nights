...
## 六类标签（每条内容标注其一，固定写法：emoji 在方括号外，如 `🔬【研究】`）
...
## 输出结构（严格按此格式）
1. 标题行：`# 🌙 AI · 一千零一夜 · 第 {tonight} 夜 ｜ {date_str}`
2. `---`
3. **No.1 主打**（今夜最惊喜的一条，精讲）：
   - 首行：`**No.1 <emoji>【标签】<项目/作品名（英文名可保留，也可起个贴切中文名）>** · <时间>`
     （时间按 published_at 换算成：今日 / 昨日 / N 天前；数据里没有时间就不标）
   - 一小段话讲清楚：这是什么 + 谁做的（数据里有作者才写，没有不编）
   - `**有点意思**：`一两句只写惊喜点（新在用法、巧在组合、戳在情感），不堆技术参数
   - 链接行：`✦ [源头](<URL>) ｜ [报道](<URL>)`
4. `---`
5. **简讯 2~3 条**（No.2、No.3、No.4，按惊喜度排列），每条两行，这是全文最后的内容（后面不加任何页脚）：
   `**No.2 <emoji>【标签】** <一句话说清这是什么、有意思在哪，尽量控制在 40 字内> · <时间>`
   `✦ [源头](<URL>) ｜ [报道](<URL>)`
   **简讯条目之间空一行**，保持呼吸感

## 链接规则（主打与简讯通用）
- 一律用 Markdown 文字链 `[源头](URL)` / `[报道](URL)`，不得输出裸 URL——长链接是视觉噪音
- 源头 = 项目/作品本体（官网、GitHub、商店页、demo 页）；报道 = 数据里同一项目的第二个链接
  （如 HN 讨论帖、演示视频页、社区帖——很多条目的 summary 字段里就带讨论链接）
- 两个链接都必须来自 JSON 数据，URL 逐字复制，不得编造；数据里确实只有一个链接时，只写 `✦ [源头](<URL>)`，省略报道部分
